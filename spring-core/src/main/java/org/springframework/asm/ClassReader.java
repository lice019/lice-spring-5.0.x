 ...
 * 该类解析符合Java类文件格式的字节数组，并为遇到的每个字段、方法和字节码指令调用给定类访问者的适当访问方法。
 *
 */
public class ClassReader {
    ...
    //要解析的类。这个数组的内容不能被修改。这个字段用于{@link Attribute}子类，类生成器或适配器通常不需要它。
    public final byte[] b;

    //{@link #b b}中每个常量池项的起始索引，加1。的
	//一个字节偏移量跳过指示其类型的常数池项标记。
