...
/**
 * AbstractCachingViewResolver它们解析的缓存视图实例的子类。缓存可以提高某些视图技术的性能。
 * 您可以通过将该cache属性设置为来关闭缓存false。此外，如果必须在运行时刷新某个视图（例如，当修改FreeMarker模板时），
 * 则可以使用该removeFromCache(String viewName, Locale loc)方法。
 *
 * @author Rod Johnson
